...
def edit_inventory(id):
    if request.method == "GET":
        ...
        query = "SELECT inventory_id, stock, inventory_description, costumes.costume_name, inventory.costume_id \
                 FROM inventory INNER JOIN costumes ON inventory.costume_id = costumes.costume_id WHERE inventory_id = %s" % (id)
        data = db.execute_query(db_connection, query).fetchall()
        ...
        if request.form.get("Edit_Inventory"): 
           ...
           # account for null stock
            if stock == "" or stock == "0":
                ...
                query = "UPDATE inventory SET inventory.stock = NULL, inventory.inventory_description = %s, inventory.costume_id = %s WHERE inventory.inventory_id = %s"
                db.execute_query(db_connection, query, (inventory_description, costume_id, inventory_id))

            # no null inputs
            else:
                # mySQL query to update the attributes of inventory with the passed id
                query2 = "UPDATE inventory SET inventory.stock = %s, inventory.inventory_description = %s, inventory.costume_id = %s WHERE inventory.inventory_id = %s"
                db.execute_query(db_connection, query2, (stock, inventory_description, costume_id, inventory_id))
